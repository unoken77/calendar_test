...
class MakeCalendars:
    ...
    def this_year_calendar_simple_with_year(self):
        output = ""
        for i in reversed(range(12)):
            month = i+1
            firstday, lastday = calendar.monthrange(self.dt_now.year, month)
            output += "↓　" + str(self.dt_now.year) + "年" + str(month) + "月\n"
            for d in reversed(range(lastday)):
                output += str(self.dt_now.year) + str(month).zfill(2) + str(d+1).zfill(2)+"\n"
            output += "↑　" + str(self.dt_now.year) + "年" + str(month) + "月\n"

        with open(file_name+str(self.dt_now.year)+".txt", mode='w', encoding='utf-8') as f:
            f.write(output)

    def this_year_calendar_without_arrows(self):
        output = ""
        for i in reversed(range(12)):
            month = i+1
            firstday, lastday = calendar.monthrange(self.dt_now.year, month)
            output += "ここより下" + str(self.dt_now.year) + "年" + str(month) + "月\n"
            for d in reversed(range(lastday)):
                output += str(self.dt_now.year) + "/" + str(month).zfill(2) + "/" +str(d+1).zfill(2) + " (" + days[(d + firstday) % 7] + ")\n"
                output += "起床。\n就寝。\n\n"
            output += "ここより上" + str(self.dt_now.year) + "年" + str(month) + "月\n"
